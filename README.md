...
設定は**テーマ 1 つだけ**。ライト / ダークの二択ではなく、色のついたテーマを
同じ 1 列から選ぶ。ライトとダークもその一員でしかない。

| ライト系 | ダーク系 |
|---|---|
| ライト（`#f2f2f7`）| ダーク（`#000000`）|
| セピア（`#f5ecdb`）| ミッドナイト（`#0d1220`）|
| セージ（`#e9f2ea`）| フォレスト（`#0a130d`）|
| スカイ（`#e6f0f4`）| プラム（`#150d15`）|
...
3. `index.html` のスウォッチに 1 行足す

```html
<button class="swatch theme-rose" data-theme="rose" aria-label="ローズ"></button>
```

スウォッチは自分のクラスから `--base` / `--tint` を読むので、色を書く場所は
CSS の 1 行だけ。追加したら**コントラストを確認すること**。既存テーマは
